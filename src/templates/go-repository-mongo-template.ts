...
export function getRepositoryMongoDBTemplate(
  name: string
) {
...
func NewMongoRepository(mongoURL, mongoDB string, mongoTimeout int) (${paramCaseName}.${pascalCaseName}Repository, error) {
...
func (r *mongoRepository) Find(code string) (*${paramCaseName}.${pascalCaseName}, error) {
...
func (r *mongoRepository) Store(redirect *${paramCaseName}.${pascalCaseName}) error {
	ctx, cancel := context.WithTimeout(context.Background(), r.timeout)
	defer cancel()
	collection := r.client.Database(r.database).Collection("redirects")
